...
## $\rm \S \, 21.1$ 网络是怎样把两台机器连起来的

### $\rm \S \, 21.1.1$ IP 地址：网络世界的门牌号
...
在[操作系统、进程与线程](../03-计算机系统/09-操作系统进程与线程.md)中你学到了端口——IP 地址告诉数据包"寄到哪栋楼"，端口告诉"楼里哪个房间（哪个进程）"。一个完整的目的地是 `IP:端口`，例如 `127.0.0.1:8000`。

IPv6（128 位）正在逐步替代 IPv4，地址写成八组十六进制数字。日常开发中绝大多数场景你接触的还是 IPv4，但知道 IPv6 的存在可以避免在见到 `::1`（IPv6 的 loopback）时困惑。

### $\rm \S \, 21.1.2$ 从你的机器到目标机器：一跳一跳地走
...
### $\rm \S \, 21.1.3$ NAT：为什么你家所有设备共用一个公网 IP
...
## $\rm \S \, 21.2$ TCP：可靠的"数据管道"

### $\rm \S \, 21.2.1$ TCP 保证什么
...
### $\rm \S \, 21.2.4$ TCP 三次握手：为什么建立连接要三个步骤
...
### $\rm \S \, 21.2.5$ TIME_WAIT：为什么端口不能立刻复用
...
### $\rm \S \, 21.2.6$ HTTP keep-alive：不用每次都三次握手
...
### $\rm \S \, 21.2.7$ tcpdump 最小实践：不是"抓包黑客工具"
...
在[开发者命令行工具箱](../02-终端与工具/06-开发者命令行工具箱.md)中你用过 `curl` 直接查看原始 HTTP 流量。现在从协议角度重新审视 `curl` 的输出：每一行响应头、状态码、响应体，都对应上面结构的一部分。

### $\rm \S \, 21.3.2$ HTTP 方法：你想让服务器做什么
...
1. IP 地址和端口分别解决什么问题？
> IP 地址标识网络中的主机（寄到哪栋楼），端口标识主机上的哪个进程（楼里哪个房间）；完整目的地是 IP:端口。

2. HTTP 方法中 GET 和 POST 的核心区别是什么？哪些方法是幂等的？
> GET 获取资源、不应有副作用（只读）；POST 创建资源、有副作用；PUT 和 DELETE 是幂等的（执行多次结果相同），POST 不是。

3. 状态码 401 和 403 分别表示什么？
> 401 表示没有提供认证凭据或凭据无效；403 表示身份已确认，但无权访问这个资源。

4. HTTP 请求头和请求体分别携带什么类型的信息？
> 头部携带关于请求/响应的元信息（Host、Content-Type、Authorization 等键值对）；请求体携带实际数据（如 POST 要创建的资源内容）。

5. 一次 HTTP 请求从发出到收到首个字节，经历了哪些主要阶段？
> DNS 解析 → TCP 三次握手 → TLS 握手（HTTPS）→ 发送请求 → 服务器处理 → 收到首个响应字节；`curl -w` 可以分段测量每段耗时。

## $\rm \S \, 21.9$ 应用与辨析

1. TCP 三次握手的目的是什么？`Connection refused` 和 `Connection timed out` 分别说明什么？
> 同步双方的初始序号、建立可靠连接；refused 表示对端端口没有进程监听（SYN 被 RST 拒绝），timed out 表示 SYN 发出后没有回复——大概率是防火墙丢包或主机不可达。

2. UDP 和 TCP 的使用场景有什么不同？
> TCP 有序、无丢失，适合 HTTP、文件传输、数据库查询；UDP 不保证送达但开销小、延迟低，适合视频通话、DNS、实时游戏——时效性大于可靠性的场景。
