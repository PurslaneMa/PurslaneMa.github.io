# $\rm Chapter \,  25$ 前端开发：让数据变成页面

> 你的论文管理器现在有 SQL 数据库和 Python 数据处理脚本。你导出了一些数据为 JSON 文件（就像[文本、编码、压缩包与常见数据格式](../02-终端与工具/05-文本编码压缩包与数据格式.md)中你学过的 JSON 格式），但每次想看论文列表，你要么打开终端用 `jq` 解析 JSON，要么用文本编辑器对着大段 JSON 用眼睛筛选。你当然可以继续这样用——但如果你要把论文数据展示给一个不写代码的人，他们需要的是一个**网页**：输入框、按钮、论文卡片、点击筛选。这些都在浏览器里，而浏览器能理解和展示的文档语言就是 HTML。

## $\rm \S \, 25.1$ 网页是什么

### $\rm \S \, 25.1.1$ 浏览器不只是"看网页的东西"

浏览器的核心工作是两件：
1. **获取资源**：通过 HTTP（[网络基础到HTTP](../04-网络/21-网络基础到HTTP.md)）从服务器下载文件——HTML 文档、CSS 样式表、JavaScript 脚本、图片、字体。
...
> **类比**：HTML 之于网页，就像 struct 定义之于 C++ 程序。`struct Paper { string title; int year; }` 描述了数据的结构——哪个字段叫什么名字、是什么类型。HTML 描述了文档的结构——哪段是标题、哪段是段落、哪里是输入框。

### $\rm \S \, 25.1.3$ 一个完整的 HTML 文档
...
- `<head>`：**元数据区**——不直接显示在页面上的信息。`<meta charset="UTF-8">` 声明字符编码（回顾[文本、编码、压缩包与常见数据格式](../02-终端与工具/05-文本编码压缩包与数据格式.md)中 UTF-8 的概念——这里如果不声明，浏览器可能用错误的编码导致中文乱码）。`<title>` 是浏览器标签页上显示的文字。
- `<body>`：**可见内容区**——所有你实际在页面上看到的东西都在这里。

### $\rm \S \, 25.1.4$ HTML 是一棵树
...
## $\rm \S \, 25.3$ JavaScript：描述"这些元素的行为"
...
### $\rm \S \, 25.3.3$ `fetch`：从 JSON 文件获取数据

上面的例子是硬编码的论文列表。真实数据通常来自后端 API（[下一章](26-后端与API.md)会写），但原理和从静态 JSON 文件读取完全相同——都是浏览器发送 HTTP GET 请求，拿到 JSON 数据。JavaScript 的 `fetch` 函数发送这个请求。
...
`fetch` 和你熟悉的 `curl` 做的是同一件事——发送 HTTP GET 请求到 `/api/papers`。区别在于：`curl` 在终端运行，输出到 stdout；`fetch` 在浏览器中运行，返回数据后动态更新 DOM。浏览器限制 `fetch` 只能向同源（同域名+同端口）发请求或需要服务器明确允许跨域——这就是[域名、HTTPS、代理与排障](../04-网络/22-域名HTTPS代理与排障.md)中 CORS 机制存在的意义。

---

## $\rm \S \, 25.4$ 组件化：当页面有五十个交互元素时
...
### $\rm \S \, 25.4.3$ React、Vue 与 Svelte
...
1. 创建一个 `index.html` 文件，包含第 25.1.3 节的最小结构。
2. 在 `<body>` 中手写一个论文列表（`<ul>` + 5 个 `<li>`）。
3. 用 `<input>` 添加搜索框，用第 25.3.2 节的 JavaScript 实现实时筛选。
...
1. 把硬编码的 `<li>` 列表替换为第 25.3.3 节的 `fetch` 方案。
2. 如果你已经写了后端（[第 26 章](26-后端与API.md)），确保后端在 `localhost:8000` 运行，前端从 API 加载真实数据。
...
> ①获取资源：通过 HTTP 从服务器下载 HTML/CSS/JS/图片。②渲染页面：把 HTML 解析为 DOM 树，应用 CSS 样式，执行 JavaScript，转换为屏幕像素。

2. HTML、CSS、JavaScript 各自负责什么？为什么需要三者分工？
> HTML 负责结构和内容（"有什么"），CSS 负责样式和布局（"长什么样"），JavaScript 负责交互和逻辑（"用户操作时怎么响应"）。三者分工让内容、外观和行为可以独立修改——改颜色不需要动 HTML，改结构不需要动 JS。

3. DOM 树是什么？它和 HTML 源码的关系是什么？
> DOM 是浏览器解析 HTML 后在内存中构建的节点树——HTML 源码是文本，DOM 是这个文本的结构化表示。JavaScript 通过遍历和修改 DOM 来动态改变页面。

## $\rm \S \, 25.8$ 应用与辨析

4. `<h1>论文列表</h1>` 和 `用 JavaScript 创建一个 `<h1>` 并插入 DOM`，最终用户看到的效果一样吗？有什么区别？
> 最终视觉效果一样。区别在于：HTML 声明式定义（页面加载时一次性构建），JavaScript 动态创建（可以在用户交互后改变）。搜索引擎和屏幕阅读器能看到 HTML 中的 `<h1>`，但不一定能看到 JavaScript 动态插入的内容。

5. 为什么浏览器中的 `fetch` 不能随意向任何域名发请求？
> 同源策略——浏览器出于安全限制，禁止一个域名的页面向另一个域名发 `fetch`。这防止了恶意网站窃取你在其他网站的数据。跨域访问需要服务器明确通过 CORS 头允许。

6. 你改了 CSS 文件，刷新浏览器没变化。可能的原因和解决方法？
> 浏览器缓存了旧的 CSS 文件。硬刷新（Ctrl+Shift+R 或 Cmd+Shift+R）跳过缓存重新请求所有资源；或在开发者工具 Network 标签页勾选 "Disable cache"。
