# $\rm Chapter \, 16$ Python 环境、Notebook 与科学计算栈
...
## $\rm \S \, 16.1$ Python 环境的三层结构

回顾[软件、运行时、SDK 与包管理器](../02-终端与工具/07-软件运行时SDK与包管理器.md)的分层模型。Python 世界中：
...
### $\rm \S \, 16.1.1$ 为什么"系统里装了一个 Python"不够
...
## $\rm \S \, 16.2$ Jupyter Notebook：一个状态化的 Python 环境

### $\rm \S \, 16.2.1$ Jupyter 不是"浏览器里的文本编辑器"
...
## $\rm \S \, 16.3$ NumPy 的 ndarray：为什么比 Python list 快 50 倍

### $\rm \S \, 16.3.1$ ndarray：同质、定长、连续内存
...
### $\rm \S \, 16.3.3$ view vs copy：这是 C++ 选手最容易踩的坑
...
## $\rm \S \, 16.5$ C/C++ 扩展：为什么 numpy 装起来比 requests 慢

[软件、运行时、SDK 与包管理器](../02-终端与工具/07-软件运行时SDK与包管理器.md)中解释了预编译包 vs 源码安装。`numpy` 包含 C/Fortran 编写的数值核心——这就是为什么：
...
下一章（第 17 章）进入 Git——把你从"手动备份代码为 `main_v2_final_fixed.cpp`"的竞赛习惯，升级到能参与多人协作的版本控制。
