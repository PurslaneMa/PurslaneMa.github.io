...
你应该已经理解进程和线程的区别（[第 9 章](../03-计算机系统/09-操作系统进程与线程.md)），知道 C++ 的基本语法。不假设你写过任何多线程代码。
...
如果 `++counter` 抛出异常（不太可能，但在其他场景中很常见——比如网络请求、文件写入），手动 `unlock()` 永远不会执行，其他线程永久等待（死锁）。`lock_guard` 利用 RAII（第 13 章讲过）——离开作用域时析构函数自动释放锁，无论怎么离开的（正常返回、异常、提前 return）。
