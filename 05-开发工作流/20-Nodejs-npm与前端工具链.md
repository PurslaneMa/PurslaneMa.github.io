# $\rm Chapter \, 20$ Node.js、npm 与前端工具链

> npm（Node.js 的包管理器，第 7 章提过）装了几百个包到 `node_modules/`。`npm run dev` 启动了开发服务器。但你敲下这些命令时——`npm install` 到底做了什么？`node_modules` 为什么这么大？`npm run dev` 为什么能跑起来？这一章回答这些问题。

## $\rm \S \, 20.1$ Node.js：浏览器之外的 JavaScript

### $\rm \S \, 20.1.1$ 同一个语言，不同的运行时

JavaScript 最初只能在浏览器中运行——你在[域名、HTTPS、代理与排障](../04-网络/22-域名HTTPS代理与排障.md)中见过的 `F12 → Console` 就是一个浏览器 JavaScript 运行时。
...
这和[编程语言范式概览](../06-编程语言/14-编程语言范式概览.md)中讲的"同一种语言在不同运行时中可用的能力可能不同"是同一个道理——JavaScript 本身相同，但"能做什么"取决于运行时提供了什么 API。

### $\rm \S \, 20.1.2$ Node.js 的三大用途
...
## $\rm \S \, 20.2$ npm：不止是"装包的"

### $\rm \S \, 20.2.1$ npm 的三个身份
...
这三者在[软件、运行时、SDK 与包管理器](../02-终端与工具/07-软件运行时SDK与包管理器.md)的六层模型中，分别对应"远程包源""包管理器客户端"和"依赖记录文件"——和 Python 的 PyPI/pip/requirements.txt 是同一个结构。

### $\rm \S \, 20.2.2$ `package.json`：不只是依赖清单
...
### $\rm \S \, 20.2.3$ `node_modules` 为什么这么大

[依赖、构建、测试与 CI](19-依赖构建测试与CI.md)中讨论过直接依赖和间接依赖——这正是 `node_modules` 膨胀的根源。
...
## $\rm \S \, 20.3$ 前端构建工具链：从源码到浏览器可执行的产物

### $\rm \S \, 20.3.1$ 为什么前端需要"构建"
...
3. **代理**：把 `/api/*` 请求转发到后端服务器（解决跨域问题——回顾[域名、HTTPS、代理与排障](../04-网络/22-域名HTTPS代理与排障.md)中的正向代理概念）。
...
### $\rm \S \, 20.3.4$ TypeScript：JavaScript + 类型
...
TypeScript 之于 JavaScript，类似于[编程语言范式概览](../06-编程语言/14-编程语言范式概览.md)中讨论的"类型标注之于 Python"——是可选的、不改变运行时行为、但大幅提升可维护性和重构安全性。
...
在[软件、运行时、SDK 与包管理器](../02-终端与工具/07-软件运行时SDK与包管理器.md)中的原则在这里同样适用：**选一套坚持用，不要叠加**。
...
有了前端工具链，你的论文管理器已经有了"壳"——但它还活在本地。浏览器里的搜索框只能搜本地假数据，JavaScript 发不出真正的 HTTP 请求。下一站是网络：你需要理解数据怎样从服务器穿越互联网到达浏览器，以及这中间每一层在做什么——从 IP 地址、TCP 连接、HTTP 协议，一直到域名解析和 HTTPS 加密。下一章[网络基础到 HTTP](../04-网络/21-网络基础到HTTP.md)就从 "两台计算机怎样找到彼此" 开始。
