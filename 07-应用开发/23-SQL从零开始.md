# $\rm Chapter \,  23$ SQL 从零开始：查询、修改与设计
...
### $\rm \S \, 23.1.5$ SQL 是一种声明式语言

在[编程语言范式概览](../06-编程语言/14-编程语言范式概览.md)中你见过声明式范式——SQL 是它的代表。你写 `SELECT title FROM papers WHERE year >= 2019 ORDER BY citations DESC`——你说的是**"我要什么"**，数据库的查询优化器决定**怎么做**（走哪个索引、用哪种 JOIN 算法）。这和 C++ 中你手写 `for` 循环逐行判断完全不同。

---

## $\rm \S \, 23.2$ 查询进阶：筛选、排序、聚合

### $\rm \S \, 23.2.1$ WHERE：不只是 `=`
...
## $\rm \S \, 23.3$ 多表设计：为什么一张表不够

### $\rm \S \, 23.3.1$ 问题：标签怎么存
...
### $\rm \S \, 23.5.4$ EXPLAIN ANALYZE：验证索引是否被使用
...
### $\rm \S \, 23.5.5$ N+1 Query：循环里放查询的经典反模式
...
### $\rm \S \, 23.5.6$ 事务隔离级别：你看到的和我在改的可能不一样
