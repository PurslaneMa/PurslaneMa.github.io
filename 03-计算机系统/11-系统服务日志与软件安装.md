# $\rm Chapter \, 11$ 日常系统管理：软件、用户、服务与日志

> 你已经知道进程是程序的一次运行实例、服务是持续运行等待请求的程序（都来自[计算机程序与开发全景](../01-基础观念/01-计算机程序与开发全景.md)），后台进程不占用终端但终端关闭时可能被挂断信号终止（[终端、Shell与命令行](../02-终端与工具/02-终端Shell与命令行.md)和[操作系统、进程与线程](09-操作系统进程与线程.md)），信号是 OS 发给进程的异步通知（[操作系统、进程与线程](09-操作系统进程与线程.md)）。现在把这些串起来，回答一个实际问题：你写了一个后端程序，怎么让它开机自动启动、崩溃后自动重启、把输出写进结构化的日志，并且不以 root 身份运行？

## $\rm \S \, 11.1$ 从"双击运行"到"开机自启"

### $\rm \S \, 11.1.1$ 前台、后台、守护进程

[终端、Shell与命令行](../02-终端与工具/02-终端Shell与命令行.md)和[操作系统、进程与线程](09-操作系统进程与线程.md)分别从操作和机制角度讲过了前台/后台。这里聚焦一条完整的升级路径：
...
每一步解决一类问题。`nohup` 解决了"终端关了程序就挂了"的问题（原理：让进程忽略[操作系统、进程与线程](09-操作系统进程与线程.md)中讲过的 `SIGHUP` 信号——终端关闭时发给它的挂断通知），但不解决"程序崩溃了谁帮我重启"。`tmux`（终端多路复用器）让你可以在一个终端里开多个"虚拟窗口"，断开后重连——但它仍然需要你手动启动程序。
...
### $\rm \S \, 11.1.2$ systemd：现代 Linux 的服务管理器
...
### $\rm \S \, 11.1.3$ 编写一个 systemd service 文件
...
# After 只决定启动顺序，不表示"依赖"——网络没起来时它也会尝试启动

[Service]
Type=simple                   # systemd 认为进程启动即就绪（最常用）
User=paper-mgr                # 以这个用户身份运行（不是 root！）
Group=paper-mgr
...
# Python 后端应用的具体写法见第 26 章
ExecReload=/bin/kill -HUP $MAINPID   # reload 时发 SIGHUP
Restart=on-failure            # 进程崩溃时自动重启
RestartSec=5                  # 等 5 秒再重启（防止反复崩溃的快速循环）
StandardOutput=journal        # stdout 进 systemd 日志
StandardError=journal         # stderr 也进 systemd 日志

[Install]
WantedBy=multi-user.target    # 正常多用户模式下启动（服务器默认运行级别）
```
...
- **StandardOutput/Error=journal**：[计算机程序与开发全景](../01-基础观念/01-计算机程序与开发全景.md)和[内存、文件与输入输出](10-内存文件与输入输出.md)花了很大篇幅讲 stdout 和 stderr 的分工。这里的配置让两个流都进入 systemd 日志（journald），你可以在一个地方同时看到正常输出和错误——比分别看两个文件方便。
- **WantedBy=multi-user.target**：确定开机自启的"触发时机"——系统进入多用户模式（服务器典型状态）时启动。

### $\rm \S \, 11.1.4$ Windows 对照：Windows Service
...
一个常见的反模式：把所有输出都标成 INFO，然后在上万行 INFO 里找不到唯一的 ERROR。更糟糕的是，用 `cout` 的默认输出（没有时间戳、没有级别）当"日志"。在[编辑器、IDE与调试器](../02-终端与工具/04-编辑器IDE与调试器.md)的调试讨论中已经说过 `cout` 调试法的局限——到了生产环境，没有结构化日志就等同于没有任何可观测性。

### $\rm \S \, 11.2.3$ journald：systemd 的统一日志系统
...
### $\rm \S \, 11.2.4$ 日志轮转与磁盘危机

在[开发者命令行工具箱](../02-终端与工具/06-开发者命令行工具箱.md)中你用过 `df -h` 看各分区的使用情况、`du -sh` 看某个目录的大小。日志是占用磁盘的元凶之一——一个不轮转的错误日志可以在几小时内填满几百 GB。
...
## $\rm \S \, 11.3$ 身份、权限与服务账户

### $\rm \S \, 11.3.1$ 不要用你的账户跑服务
...
## $\rm \S \, 11.4$ 软件安装的"日常管理"视角

[软件、运行时、SDK与包管理器](../02-终端与工具/07-软件运行时SDK与包管理器.md)从概念上分层讲解了包管理器——系统级（apt/winget）、项目级（pip/npm）、环境级（venv/nvm），以及 PATH 如何决定哪个可执行文件被找到。这里加上实际操作和维护：

### $\rm \S \, 11.4.1$ 系统级：各平台的日常操作
...
### $\rm \S \, 11.4.2$ 源码安装：`./configure && make && make install`
...
### $\rm \S \, 11.4.3$ 动态库路径：`ldconfig` 和 `LD_LIBRARY_PATH`
...
> **Windows 对应**：DLL 的搜索顺序是当前目录 → System32 → PATH 中的目录。把 DLL 放到可执行文件同目录下是一种简单但粗暴的做法。

---

## $\rm \S \, 11.5$ 配置与 Secret 管理
...
### $\rm \S \, 11.5.2$ 不要把 Secret 放进 Git

[软件、运行时、SDK与包管理器](../02-终端与工具/07-软件运行时SDK与包管理器.md)讲过"安装包相当于运行一段不受审查的代码"。这里给出具体做法：
...
### $\rm \S \, 11.5.3$ 版本控制和变更记录

[Git与团队协作](../05-开发工作流/17-Git与团队协作.md)一章会完整讲 Git，但这里先说一个习惯：对系统配置文件的每次修改都记录在注释中（日期、修改人、原因）。这不算"完美的配置管理"，但在 team 只有两三个人时有效避免了"上次是谁改的这个参数"的追问。

---

## $\rm \S \, 11.6$ 定时任务：让机器按时自动做事

### $\rm \S \, 11.6.1$ cron：最经典的定时系统
...
### $\rm \S \, 11.6.2$ systemd timer：cron 的现代替代
...
[Timer]
OnCalendar=daily
Persistent=true         # 如果上次错过了（机器关机），开机后补执行
RandomizedDelaySec=300  # 随机延迟最高 5 分钟
```

### $\rm \S \, 11.6.3$ Windows 任务计划程序
...
## $\rm \S \, 11.7$ 故障演练：故意破坏，练习修复

以下五个场景覆盖了 80% 的服务故障。在一个安全环境中（虚拟机或临时云主机）亲手制造并排查每一个。

### $\rm \S \, 11.7.1$ 场景一：服务启动失败
...
### $\rm \S \, 11.7.2$ 场景二：端口被占用

启动第二个绑定同一端口的进程。用[开发者命令行工具箱](../02-终端与工具/06-开发者命令行工具箱.md)中学过的 `ss -tlnp` 或 `Get-NetTCPConnection` 找出占用者。

### $\rm \S \, 11.7.3$ 场景三：权限不足
...
### $\rm \S \, 11.8.1$ 实践一：编写并管理 systemd service

1. 写一个简单的 Python 脚本：一个无限循环，每秒打印一行时间戳和"alive"。
2. 创建 systemd service 文件，像第 11.1.3 节那样配置。
3. `systemctl start` → 检查 `systemctl status` → `journalctl -u` 查看输出。
4. `systemctl enable` 并重启机器（如果是虚拟机），确认服务在开机后自动运行。
5. 手动 `kill` 进程，等待 `RestartSec` 后观察 `systemctl status`——服务应该已经自动重启。
6. 清理：`systemctl stop`、`systemctl disable`、删除 service 文件、`systemctl daemon-reload`。

### $\rm \S \, 11.8.2$ 实践二：分析日志

1. 用 journalctl 查看你的服务的日志，尝试不同的过滤参数。
2. 用 `-p err` 只看错误级别的日志。
3. 用 `--since` 和 `--until` 限制时间范围。
4. 导出为文本保存：`journalctl -u paper-manager > service.log`。

### $\rm \S \, 11.8.3$ 实践三：创建服务账户

1. 用 `useradd -r` 创建一个系统账户。
2. 修改第 1 个实践中的 service 文件，用这个账户运行。
3. 启动服务，用 `ps aux | grep paper` 确认进程的所有者是新账户而不是你的用户。
...
1. `systemctl start`、`stop`、`restart`、`reload`、`enable` 分别做什么？
> start 启动、stop 停止、restart 停掉再启动、reload 发 SIGHUP 让进程重读配置（不重启进程）、enable 设置开机自启（disable 取消）。

2. systemd service 文件中 `Restart=on-failure` 和 `RestartSec` 的作用是什么？
> 进程崩溃（退出码非 0）时自动重启；RestartSec 是重启前的等待秒数，防止反复崩溃时进入快速循环。

3. 为什么不应该以 root 身份运行应用服务？
> root 能绕过几乎所有权限检查：服务一旦被攻破，攻击者可以直接修改系统文件；用专用低权限账户运行，把损害限制在服务自身。

4. 日志级别 DEBUG、INFO、WARN、ERROR、FATAL 分别适合什么场景？
> DEBUG 是调试细节（生产关闭）；INFO 是正常关键事件；WARN 是潜在问题（如磁盘 80% 满）；ERROR 是操作失败需要响应；FATAL 是服务不可恢复（通常退出）。

5. `journalctl -u service-name -f` 在做什么？
> 只看该服务的日志，并实时追踪新增内容（-f 类似 tail -f）。

6. umask 影响什么？如何设置服务创建的新文件的默认权限？
> 它决定新创建文件的默认权限（满权限 666/777 减去掩码）；在 systemd unit 中写 `UMask=027` 即可让服务的新文件默认组内可读、其他用户不可读。

7. `ldconfig` 解决什么问题？
> 重建系统的动态库缓存（按 /etc/ld.so.conf 配置的目录），让运行时能找到新安装的 .so；`ldconfig -p` 可查询某个库是否在缓存中。

## $\rm \S \, 11.11$ 应用与辨析

8. cron 和 systemd timer 的优缺点分别是什么？
> cron 简单经典、到处可用，但日志分散（靠邮件）、没有依赖管理；systemd timer 与 journald 集成、支持随机延迟（避免"午夜风暴"）和错过后的补执行（Persistent=true）。

9. 你遇到了一个服务启动失败的错误。按什么顺序收集证据？
> `systemctl status 服务名` 看状态和退出码 → `journalctl -u 服务名 -n 20` 看最近日志（第一行常直接说明原因）→ 再检查路径、端口、权限和动态库（ldd）是否齐全。

10. 怎样确保 API Key 不会通过代码被提交到 Git 仓库？
> 不放源码和命令行：放环境变量或仓库之外的 600 权限文件（systemd 用 EnvironmentFile 加载）；`.env` 加入 .gitignore 并提供 .env.example 占位模板；一旦泄露立即轮换密钥。
