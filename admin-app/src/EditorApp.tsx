...
export function EditorApp(){
 ...
 const save=async()=>{if(!selected){setError('请先选择一篇文章');return}try{const body={title:selected.title,date:selected.date||new Date().toISOString(),_content:content(),categories:selected.categories,tags:selected.tags,references:selected.references||[],note_id:selected.note_id};await api(mode+'/'+selected._id,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});setStatus('已保存');await load()}catch(e){setError(e instanceof Error?e.message:'保存失败');setStatus('保存失败')}}
