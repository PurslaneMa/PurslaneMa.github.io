...
pip 的工作流程：你告诉它"我要 requests" → 它连接 **PyPI**（Python Package Index，`pypi.org`，Python 社区的中央包仓库）→ 下载 `.whl` 文件 → 解压到 Python 的 `site-packages/` 目录 → 检查依赖 → 一并安装依赖。
...
`import` 能成功的前提是：pip 安装的目录在 Python 的搜索路径（`sys.path`）中。每个 Python 安装有自己的 `site-packages/`——系统里有两个 Python 时，A 的 pip 装的包，B 的 `import` 一定找不到。

### $\rm \S \, 7.1.2$ 最常见的陷阱：pip 和 python 不是一对
...
> **经验规则**：忘掉 `pip install`——养成 `python -m pip install` 的习惯。它不是"高级写法"，是唯一不会装错 Python 的写法。
...
| **项目环境** | 某个项目使用的特定运行时版本、依赖版本和配置的组合 | 虚拟环境（venv）、`node_modules/`、`package.json` |
...
系统包管理器通常需要**管理员权限**（`sudo` 或 UAC 弹窗），因为它们把文件写到系统目录（`/usr/`、`C:\Program Files\`）。安装完成后，可执行文件放在系统 PATH 中的目录，所有用户通常都能找到。

**什么时候用系统包管理器**：安装你在任何项目中都需要的工具——`git`、`g++`、`curl`、`make`、`python`。

### $\rm \S \, 7.3.2$ 语言/项目级包管理器

管理**特定编程语言的第三方库**：

| 语言/生态 | 包管理器 | manifest 文件 | lockfile | 依赖目录 |
|---|---|---|---|---|
| Python | pip / uv | `requirements.txt` 或 `pyproject.toml` | `requirements-lock.txt`（需额外工具） | `venv/lib/` |
...
| Java | Maven / Gradle | `pom.xml` / `build.gradle` | — | `~/.m2/` (Maven) |

项目级包管理器通常**不需要管理员权限**——它们把包安装到项目目录或用户目录中。
...
四层不是互相替代的关系——它们解决不同粒度的问题。一个典型项目中你可能同时用到全部四层：用 `apt` 装的 g++，用 `pip`（在 venv 中）装的 numpy，用 `nvm` 切换的 Node.js 版本，最后用 Docker 打包成镜像发给团队。

### $\rm \S \, 7.3.5$ Windows 包管理器：winget、Chocolatey 与 Scoop

Windows 不像 Linux——没有统一的 `apt`。但你有三种选择：

| 工具 | 特点 | 权限 |
|------|------|------|
| **winget** | 微软官方，Win10/11 自带，包来自社区仓库 | 需要管理员 |
...
**Scoop**（开发者首选——不需要 UAC 弹窗）：

```powershell
Set-ExecutionPolicy RemoteSigned -Scope CurrentUser
irm get.scoop.sh | iex            # 安装（不需要管理员！）
...
## $\rm \S \, 7.4$ 命令"python"到底指向哪里

这是诊断所有安装问题的核心线索。

### $\rm \S \, 7.4.1$ 回顾 PATH 的搜索机制

在[终端、Shell与命令行](02-终端Shell与命令行.md)中讲过 PATH 的搜索机制：当你在终端输入 `python`，Shell 按照 PATH 环境变量列出的目录顺序，逐个查找名为 `python`（或 `python.exe`）的可执行文件，找到第一个就停止。

问题来了——如果你的系统中有多个 Python 安装：
- `C:\Python312\python.exe`（手动安装）
- `C:\Users\Alice\AppData\Local\Programs\Python\Python39\python.exe`（另一个版本）
- `C:\Users\Alice\anaconda3\python.exe`（Anaconda 带的）
- `C:\Users\Alice\.venv\myproject\Scripts\python.exe`（venv 的）

哪个会先被找到？取决于 PATH 中目录的排列顺序。

### $\rm \S \, 7.4.2$ 诊断：看看你到底在调用哪个

在[终端、Shell与命令行](02-终端Shell与命令行.md)中你学过 `Get-Command` 和 `command -v`——它们回答"输入的命令名实际解析到了哪个可执行文件"。当"版本不对"时，这是第一步：
...
> **关键习惯**：每次排查"版本不对"的问题时，**同时记录版本号和可执行文件路径**。只记录版本号不够——你的系统里可能有两个"3.12.0"但装在不同位置、属于不同环境。

### $\rm \S \, 7.4.3$ 典型的版本冲突场景
...
## $\rm \S \, 7.5$ 全局安装 vs 项目内安装

### $\rm \S \, 7.5.1$ 全局安装的后果

```bash
pip install requests    # 装到了系统 Python 的 site-packages 中
```
...
pip install requests    # 装到 venv 中，不影响系统 Python
...
# 在新机器上重建
npm install    # 根据 lockfile 安装精确版本
```
...
## $\rm \S \, 7.6$ manifest 和 lockfile：为什么有两个文件
...
两者都提交到 Git（lockfile 也要提交——它是团队环境一致性的保障）。`.gitignore` 中排除的是依赖目录本身（如 `node_modules/`、`venv/`），因为 lockfile 已经包含了重建它们的全部信息。

---

## $\rm \S \, 7.7$ 从源码到二进制：包是怎么来的

### $\rm \S \, 7.7.1$ 预编译包 vs 源码包

当你运行 `pip install numpy`，pip 背后做了两件事之一：
...
`numpy` 内部包含用 C 写的数值计算核心——比纯 Python 快几十到几百倍。这就是为什么安装 `numpy` 时有时候很快（有预编译包），有时候要等几分钟并可能需要装编译器。

### $\rm \S \, 7.7.2$ 为什么"源码安装失败"很常见
...
> **经验规则**：优先找预编译包（wheel、prebuilt binary）。如果必须从源码构建，做好它需要编译器、相关库和 5-15 分钟编译时间的准备。编译错误的第一行通常已经指出了缺少什么。

---

## $\rm \S \, 7.8$ 升级、降级与卸载：状态管理的坑

### $\rm \S \, 7.8.1$ 升级不是"装新版本就行"

```bash
pip install --upgrade numpy     # 升级单个包
pip install -r requirements.txt # 按 manifest 重建——不会自动卸载不再需要的包
```
...
### $\rm \S \, 7.8.2$ 卸载不等于清理干净

```bash
pip uninstall numpy           # 删除了包文件
...
## $\rm \S \, 7.10$ 贯穿项目：论文管理器的环境搭建

假设论文管理器项目使用 Python 后端 + Node.js 前端工具链。初始化流程应该是：
...
# 4. 验证环境
python -c "import flask; print(flask.__version__)"    # 后端框架
node -e "console.log(require('./package.json').name)" # 前端项目名
```

完成上述四步后，任何人在任何机器上拿到这个项目（只要有 Python 和 Node.js），都能在 2 分钟内重建完全相同的开发环境。
...
1. 记录版本号。
2. 记录可执行文件完整路径。
3. 查看 PATH 环境变量，确认你找到的可执行文件位于 PATH 的哪个目录中。
4. 如果存在多个版本，把它们列出来并按 PATH 顺序排列。

### 实践二：创建虚拟环境

1. 在安全目录中用 `python -m venv testenv` 创建虚拟环境。
2. 激活后，确认 `command -v python`（或 `Get-Command python`）指向 `testenv` 内的可执行文件。
3. 在环境中 `pip install requests`。
4. `pip freeze` 查看已安装的包列表。
5. 退出环境（`deactivate`），再次查看 `command -v python`——确认它回到了系统 Python。
6. 删除 `testenv` 目录——整个环境就清除了，系统 Python 不受任何影响。

### 实践三：读懂 manifest

1. 找任意一个开源 Python 或 Node.js 项目，打开它的 `requirements.txt` / `pyproject.toml` 或 `package.json`。
2. 找出项目的三个直接依赖。
3. 看一眼 lockfile（如果存在），观察它的大小——那些几百行的内容，大部分是依赖的依赖的锁死版本。
4. 在项目的 README 中找到环境搭建说明，对比是否和本节描述的流程一致。
...
- **PATH 决定 Shell 找到哪个可执行文件**。排查"安装了找不到"的第一步永远是 `command -v`（或 `Get-Command`）。
...
1. 运行时和 SDK 有什么区别？
> 运行时是执行某类程序的引擎（Python 解释器、Node.js、JVM）；SDK 是开发某平台所需的工具集合，通常包含编译器、运行时、库、调试器和文档。

2. 系统包管理器和语言包管理器分别管理什么？什么时候用哪个？
> 系统包管理器管理操作系统层面的软件（git、g++、curl），需要管理员权限；语言包管理器管理某语言的第三方库，装到项目或用户目录；工具用系统的装，项目依赖用语言的装。

3. 虚拟环境解决的核心问题是什么？它怎么做到的？
> 隔离不同项目的运行时版本和依赖版本，互不冲突；它创建独立的目录树并修改 PATH，让当前 Shell 优先找到隔离环境中的 python/pip。

4. manifest 和 lockfile 分别描述什么？为什么两者都要提交到 Git，但依赖目录要排除？
> manifest 声明"需要什么、版本范围"（人的意图）；lockfile 记录"精确安装了哪个版本"（含间接依赖）；提交两者保证团队和 CI 重建完全一致的环境，而依赖目录可以用 lockfile 重建，所以排除。

5. `pip install` 下载预编译包和从源码构建有什么区别？
> 预编译包（wheel）已针对当前平台编译好，安装快、不需要本地编译器；从源码构建需要本地编译器（如 VS Build Tools）和相关库，慢且更容易失败。

6. 为什么说安装包相当于运行了一段不受审查的代码？
> 安装过程可能执行包作者编写的脚本（setup.py、postinstall），这些脚本可以做任何事；包管理器只做基础扫描，所以只装来源明确的包、警惕拼写仿冒（typosquatting）。

## $\rm \S \, 7.14$ 应用与辨析

7. `python` 和 `pip` 可能来自不同的安装——为什么这是问题？怎么防止？
> 用 `pip` 装的包装进了一个 Python 的环境，而 `python` 命令实际是另一个 Python，`import` 自然找不到；用 `python -m pip install` 保证 pip 属于当前 `python`。

8. 一个新队友拿到你的项目后，应该按什么顺序重建开发环境？
> 先装运行时（系统包管理器）→ 创建虚拟环境 → 按 manifest/lockfile 安装依赖（`pip install -r` / `npm ci`）→ 验证关键包能导入、服务能启动。

---

到这里，你已经具备了"在自己的机器上独立操作文件、终端、编辑器、包管理器和开发环境"的基础能力。下一章[从竞赛机模型到真实硬件](../03-计算机系统/08-计算机硬件速通.md)将镜头从操作层面拉到机器内部——CPU 为什么有时候"看起来很快"但实际上不快？你竞赛时建立的硬件模型有哪些关键细节需要修正？
