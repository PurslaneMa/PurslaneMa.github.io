...
- 很多开发工具用 YAML 做配置（Docker Compose——第 27 章，GitHub Actions——第 19 章）。你不需要现在就记住全部语法，但要知道"缩进对齐不要混 Tab"是 $90\%$ YAML 错误的根源。

### $\rm \S \, 5.5.4$ TOML：给配置文件设计的干净格式
...
| 格式 | 命令行工具 | 用途 |
|---|---|---|
| JSON | `jq` | 查询、过滤、变换 JSON（见[开发者命令行工具箱](06-开发者命令行工具箱.md)） |
...
> 不在于字节本身，而在于解读方式：文本按字符编码解读为可读字符，二进制按特定格式（图片、可执行文件等）解读。

2. 同一个字节 `0xD6` 在 GBK 和 UTF-8 中可能代表什么不同的东西？
> 字节本身没有含义，含义由编码决定：在 GBK 中它可能是某个汉字字节序列的一部分，在 UTF-8 中可能是另一个字符的一部分；用错的编码解读就是乱码。

3. UTF-8 为什么在处理纯英文时和 ASCII 完全兼容？
> 设计如此：ASCII 字符（U+0000–U+007F）在 UTF-8 中仍用单个字节表示且编码值与 ASCII 完全一致，所以纯英文的 UTF-8 文件和 ASCII 文件字节相同。

4. BOM 是什么？UTF-8 为什么不需要它？
> BOM 是文件开头的 2-3 个不可见字节，用于标记编码或字节序；UTF-8 没有字节序问题，BOM 反而会出现在数据开头导致解析失败（如脚本首行、JSON）。

5. `\r\n` 和 `\n` 分别在哪些系统中是默认换行符？
> Windows 用 `\r\n`（CRLF），Linux 和现代 macOS 用 `\n`（LF）。

6. `tar` 和 `gzip` 分别做什么？`.tar.gz` 的两层扩展名意味着什么？
> tar 打包（把多个文件合并成一个，不压缩），gzip 压缩（减小体积）；`.tar.gz` 表示先 tar 打包再 gzip 压缩，用 `tar -xzvf` 一步解压。

7. JSON 的尾随逗号为什会导致解析失败？
> JSON 语法禁止最后一个元素后面出现逗号（Python 字典允许，JSON 不允许），解析器遇到就会报格式错误。

8. 正则表达式 `\d{3}-\d{4}` 匹配什么？`*` 和 `+` 的区别是什么？
> 匹配"3 个数字-4 个数字"（如 123-4567）；`*` 表示前一个模式出现零次或多次，`+` 表示至少出现一次。

9. SHA256 哈希验证能告诉你的两件事分别是什么（能做什么和不能做什么）？
> 能验证文件完整性——与发布方提供的值一致说明文件没被改动；不能验证来源可信——签名的恶意文件也有正确哈希，来源验证需要数字签名。

## $\rm \S \, 5.12$ 应用与辨析

10. CSV 中，值本身包含逗号时应该怎么处理？
> 把整个字段用双引号包起来；不要手写解析器，用 Python 的 `csv` 模块或 PowerShell 的 `Import-Csv` 处理。

---

下一章[开发者命令行工具箱](06-开发者命令行工具箱.md)不再讲"理论"，而是给你一个实战工具箱——你将在终端中用一行命令搜索代码、请求 API 并定位端口冲突，而不用打开文件管理器或浏览器。
