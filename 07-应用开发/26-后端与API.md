# $\rm Chapter \,  26$ 后端与 API：让前端有数据可拿
...
在[操作系统、进程与线程](../03-计算机系统/09-操作系统进程与线程.md)中你学过单线程 vs 多线程。后端框架对并发的处理方式不同：
...
关键在两点：`key` 是 `idempotency` 表的主键（唯一性约束在[SQL 从零开始](23-SQL从零开始.md)学过），重复插入抛 `IntegrityError`，整个事务回滚——重复的论文不会写入，然后取回第一次的结果返回。

客户端配合幂等键重试（`requests` 是 Python 的 HTTP 客户端库，第 15 章装过）：
...
- 4xx 不重试：请求本身有错，重试一万次结果一样；唯一的例外是 **429 Too Many Requests**——"你太快了"，等一会儿再试是合理的（§26.9 讲限流时细说）。
...
## $\rm \S \, 26.8$ 优雅关闭（Graceful Shutdown）：给进程一点收尾时间

在[操作系统、进程与线程](../03-计算机系统/09-操作系统进程与线程.md)学过：`kill PID` 发 SIGTERM，进程可以注册处理函数在退出前做清理；`kill -9` 的 SIGKILL 不可拦截。[系统服务与日志](../03-计算机系统/11-系统服务日志与软件安装.md)的 `systemctl stop`、以及第 27 章会讲的 `docker stop`，发的都是 SIGTERM——它们给你收尾的时间。但 Flask 默认不处理 SIGTERM：收到就立刻终止，正在处理的请求被拦腰截断（用户拿到连接中断而不是响应），数据库事务可能只写了一半。

第 9 章提到过的优雅退出（graceful shutdown）——后端语境里叫**优雅关闭**——完整形态是收到 SIGTERM 后按顺序做四件事：停止接受新请求；等待正在处理的请求完成（设一个超时上限，不能无限等）；关闭数据库连接（提交或回滚未完成的事务）；进程退出。
...
## $\rm \S \, 26.9$ 限流（Rate Limiting）：别让一个客户端独占服务器

认证回答"你是谁"，不回答"你能用多快"。一个失控脚本可以每秒打 1000 个请求：把数据库打满让所有用户卡住、把按调用量计费的服务烧光预算、把整站数据爬走。[域名、HTTPS、代理与排障](../04-网络/22-域名HTTPS代理与排障.md)提到反向代理能做的"限流"，就是这里的主角：**限流**（rate limiting）限制同一客户端单位时间内的请求数，超出直接拒绝。
...
最小实现——把限流挂在 `before_request` 钩子上（26.5 节的中间件位置，每个请求进入路由前先执行）：
...
被拒的请求收到 **429 Too Many Requests**（状态码表在[网络基础到HTTP](../04-网络/21-网络基础到HTTP.md)见过）和 **Retry-After** 响应头——后者告诉客户端等多少秒再试，正好呼应上一节"429 值得重试"：客户端该做的是等够时间再试，而不是立刻再打。

两点边界：内存字典只对单个进程有效——多进程、多台机器必须用共享存储（Redis 是常见选择），否则每个进程各算各的额度（本机演示中所有请求都来自 127.0.0.1，额度是所有本地客户端共用的）；生产环境更常见的做法是在 Nginx（[域名、HTTPS、代理与排障](../04-网络/22-域名HTTPS代理与排障.md)学过它做反向代理）里配置，后端代码一行不用改：
...
`rate=60r/m` 是平均速率，`burst=20` 允许短时突发（令牌桶思想）。Nginx 位于后端之前（第 22 章的部署图），请求先经过它，限流自然生效。

---

## $\rm \S \, 26.10$ API 版本管理（Versioning）：改接口不能砸了老客户端
...
## $\rm \S \, 26.11$ API 调试工具：curl 之外的图形化客户端
...
1. 给 `POST /api/papers` 加幂等键（§26.7 的代码）：同一个 key 发两次 `curl`，验证两次返回的 `id` 相同、`papers` 表里只有一行。
2. 加上限流中间件（§26.9 的代码）：循环快速发 70 个请求，观察第 61 个起返回 429 并带 `Retry-After` 头。
3. （Linux/macOS）启动后另开终端 `kill <PID>`：在途请求正常返回、新请求返回 503，验证优雅关闭（§26.8）。
...
> 路由匹配（URL→处理函数）、参数校验（确保输入合法）、业务逻辑（查数据库/计算）、序列化（对象→JSON）、错误处理（合适的状态码和错误消息）。

2. REST API 的核心约定是什么？（路径、方法、状态码）
> 路径用名词（`/api/papers`）、HTTP 方法表达动作（GET/POST/PUT/DELETE）、状态码表达结果（200/201/400/404/500）、JSON 做数据格式。

3. 中间件的作用是什么？
> 在请求到达处理函数之前和之后，串行执行通用逻辑（日志、认证、CORS、压缩），避免在每个处理函数中重复代码。

4. 客户端重发 POST 请求为什么危险？幂等键怎么消除这个危险？
> POST 不幂等，服务器可能已经处理过第一次请求（只是响应丢了），重发会重复创建。幂等键是客户端为一次业务动作生成的 UUID，服务器以它为表主键：同一个 key 只处理一次，重复请求触发主键冲突，直接返回第一次的结果。

5. 收到 SIGTERM 后，后端按什么顺序收尾？为什么不能无限等？
> 停收新请求 → 等待在途请求完成（设超时上限）→ 关闭数据库连接 → 退出。调用方（如 Docker stop）默认等 10 秒就会补发 SIGKILL 强杀，收尾必须在此时间内完成。

## $\rm \S \, 26.15$ 应用与辨析

4. `GET /api/papers/42` 和 `DELETE /api/papers/42` 的 URL 相同，后端怎么区分？
> 通过 HTTP 方法区分——Flask/FastAPI 的路由装饰器同时绑定路径和方法（`@app.route('/api/papers/<id>', methods=['DELETE'])`）。

5. JWT 的 payload 可以放密码吗？为什么？
> 不能。JWT payload 只是 Base64 编码（非加密），任何人都能解码读取。JWT 的安全靠签名（防篡改），不靠加密（防读取）。敏感信息永远不放 JWT。

6. 用户提交了一个不存在的 tag 值，后端应该返回什么状态码？为什么不是 500？
> 200 OK + 空列表（[]）。这不是错误——查询条件有效，只是没有匹配的数据。500 表示"服务器内部出了问题"，用户传了合法但无匹配的参数不是服务器的错。

7. 你做了"每分钟 60 次"的固定窗口限流，却发现有客户端在 2 秒内发出了 120 个请求。漏洞在哪？怎么修？
> 固定窗口的边界漏洞：第 59 秒用光当前窗口额度，第 61 秒进入新窗口又满额，两个窗口的额度在 2 秒内叠加。改用滑动窗口（统计最近 60 秒）或令牌桶（平均速率 + 突发额度）。
