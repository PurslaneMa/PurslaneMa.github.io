# $\rm Chapter \, 15$ 面向 C++ 选手的 Python 速通
...
### $\rm \S \, 15.2.3$ 切片：Python 最强大的内置操作
...
列表推导本质是用表达式描述"我要一个什么形状的结果"，更接近[编程语言范式概览](14-编程语言范式概览.md)中讨论的声明式风格。Python 也支持字典推导和集合推导：
...
### $\rm \S \, 15.3.3$ `pip` 和 `venv`——你已经在 [软件、运行时、SDK 与包管理器](../02-终端与工具/07-软件运行时SDK与包管理器.md) 中理解了概念
...
## $\rm \S \, 15.5$ 上下文管理器：Python 版的 RAII（但有限）

[从竞赛 C++ 到工程 C++](13-C++从竞赛到工程.md) 中详细讲了 RAII——对象析构时自动释放资源。Python 没有析构函数的确定性调用（垃圾回收时机不可预测），所以提供了 `with` 语句：
...
## $\rm \S \, 15.6$ 类型标注：让 Python 代码可静态检查
...
## $\rm \S \, 15.7$ 贯穿项目：C++ 论文工具 → Python 重写
...
# dict | None：联合类型（Python 3.10+），等价于第 15.6 节的 Optional[dict]
...
## $\rm \S \, 15.8$ 不只是 CPython：Python 实现与工具
...
下一章（第 16 章）进入 Python 的生态系统：Jupyter Notebook 是什么（它不是"浏览器里的 Python 编辑器"）、NumPy 的 ndarray 为什么比 Python list 快 50 倍、科学计算栈（pandas/Matplotlib/scikit-learn）各自负责什么——以及 AI 初学者最常遇到的环境混乱问题。
