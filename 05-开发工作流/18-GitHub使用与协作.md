# $\rm Chapter \, 18$ GitHub：远程仓库与协作

> [上一章](./17-Git与团队协作.md)中的一切操作都在你自己的电脑上完成——只有你能看到那些提交。当你需要和队友协作、备份代码或在开源社区贡献时，需要一个远程仓库。GitHub 是最广泛使用的 Git 远程托管平台，但它不是 Git 本身——它是在 Git 之上添加了协作层、权限管理和自动化的服务。

## $\rm \S \, 18.1$ Git、GitHub、本地仓库和远程仓库的关系
...
## $\rm \S \, 18.2$ 第一次连接：从 `git clone` 开始
...
### $\rm \S \, 18.2.2$ HTTPS vs SSH 认证

GitHub 需要验证"你确实是你"。两种方式：

| 方式 | URL 格式 | 优点 | 缺点 |
|---|---|---|---|
| **HTTPS**（加密 HTTP——详见第 22 章） | `https://github.com/USER/REPO.git` | 无需额外配置，任何网络环境通常都通 | 每次 push 要输用户名和 Token（可以用 Git 凭据管理器缓存） |
| **SSH** | `git@github.com:USER/REPO.git` | 配置一次后无需重复认证 | 需要生成 SSH 密钥对并上传公钥 |

SSH 密钥配置（在[SSH远程开发与服务器生存指南](../03-计算机系统/12-SSH远程开发与服务器生存指南.md)中你学过 SSH 远程连接的概念）：
...
**Token 和私钥永远不要出现在代码、命令行参数或环境变量中**——这是从[软件、运行时、SDK 与包管理器](../02-终端与工具/07-软件运行时SDK与包管理器.md)的安全原则在 Git 层面的延伸。GitHub 的 Personal Access Token 按仓库权限细分，定期过期——比你用主密码更安全。

---

## $\rm \S \, 18.3$ 远程协作的核心命令

### $\rm \S \, 18.3.1$ `fetch`、`pull`、`push`
...
## $\rm \S \, 18.4$ Issue：不只是 Bug 报告

**Issue** 是 GitHub 的讨论线索。它可以用来：
- 报告 Bug（附最小复现步骤——回想[编辑器、IDE 与调试器](../02-终端与工具/04-编辑器IDE与调试器.md)中写最小复现的流程）
- 提议新功能
- 提问和讨论
...
## $\rm \S \, 18.5$ Pull Request：文明的代码合并方式

### $\rm \S \, 18.5.1$ PR 是什么

在[上一章](./17-Git与团队协作.md)中你直接在本地 `git merge`。在一个团队项目中，直接合并到 `main` 意味着一个人的错误会立刻影响所有人。
...
## $\rm \S \, 18.6$ GitHub Actions：推送后自动发生的事情
...
## $\rm \S \, 18.7$ `.gitignore` 和 Secret

### $\rm \S \, 18.7.1$ `.gitignore`——永远不要让这些东西进入 Git
...
其中 `gh pr checkout 42` 把 PR #42 的分支拉到本地并切换过去——审查别人的 PR 不再需要先到网页上找分支名；`gh run watch` 对应 §18.6 的 GitHub Actions——推送后不用刷新网页，直接在终端实时看 CI 每一步的输出。
...
> Git 是本地运行的版本控制工具；GitHub 是托管 Git 仓库并提供 Issue、Pull Request、Actions 等协作功能的网站/服务（GitLab、Gitee 类似）。

2. `git fetch`、`git pull`、`git push` 分别做了什么？
> fetch 从远程下载新提交但不改动工作区；pull = fetch + merge（拉取并合并到当前分支）；push 把本地提交推送到远程。

3. clone 和 fork 有什么区别？什么时候用 fork？
> clone 是拿到仓库的本地副本；fork 是把别人的仓库完整复制到你自己的 GitHub 账户下（不影响原仓库）；向原项目贡献代码时用 fork + PR。

4. Pull Request 在合并之前做了什么？为什么需要它？
> 在合并前插入人类审查环节：查看 diff、讨论、请求修改、批准后才合并；它是代码质量保障，也让多人共享代码的所有权。

5. GitHub Actions 的核心价值是什么？
> 每次 push 或创建 PR 时自动在干净虚拟机中安装依赖、构建、运行测试——项目的"云端 OJ"，失败会显示红叉并可用分支保护阻止合并。

## $\rm \S \, 18.12$ 应用与辨析

1. 密钥被推送到公开仓库后，第一步应该做什么？
> 立即轮换密钥——在服务提供商处生成新密钥使旧密钥作废；从 Git 历史中删除只是补救，不能保证没有人已经 clone 了仓库。

2. HTTPS 和 SSH 两种远程认证方式分别适合什么场景？
> HTTPS 无需额外配置、受限网络通常也通，但每次 push 需要用户名和 Token（可交给凭据管理器缓存）；SSH 需要先生成密钥对并上传公钥，配置一次后免重复认证——长期个人开发常用 SSH，临时环境或网络受限时用 HTTPS。

---

到这里，你已经能独立管理本地和远程仓库、参与团队协作。接下来，你写的代码不能一直靠手动 `g++` 和 `python script.py`——需要系统的依赖管理、构建自动化和测试体系。下一章比[软件、运行时、SDK 与包管理器](../02-终端与工具/07-软件运行时SDK与包管理器.md)更进一步，把依赖、构建、测试和 CI 串成完整的开发流水线。
