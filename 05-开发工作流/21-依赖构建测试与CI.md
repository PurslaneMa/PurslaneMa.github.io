...
> - □ 写过包含多个文件的项目（第 15 章起）
> - □ 安装并使用过第三方依赖（第 9 章）
> - □ 在终端运行过构建或测试命令（第 15 章 §15.1.2、§15.7）

## $\rm \S \, 21.1$ 构建：不只 `g++ main.cpp`

### $\rm \S \, 21.1.1$ 构建系统的统一视角

CMake 是 C++ 生态中最通用的构建系统，但不是唯一的，也不是每种语言都需要它。

**构建系统**（build system）的核心任务是：读取你对“如何从源码生成产物”的描述，确定依赖关系，然后只对变化的部分重新构建。

你在[第 15 章 C++ 工程](../06-编程语言/15-C++从竞赛到工程.md)中已经用过 CMake——在 `CMakeLists.txt` 中声明源文件和编译选项，CMake 生成 Makefile 或 Ninja 文件，再由 Make/Ninja 调用 `g++` 编译。这是 C++ 生态最通用的方案，也是你目前唯一需要掌握的构建系统。
...
### $\rm \S \, 21.1.2$ 任务运行器：当“构建”不只是一次编译

你的项目中可能混合了多种语言——C++ 的 CLI 用 CMake，Python 的后端需要 `pip install`，前端（以后会加入，详见[第 22 章 Node.js 与前端工具链](22-Nodejs-npm与前端工具链.md)）需要 `npm run build`。你不想手动跑三次不同的命令。

**任务运行器**（task runner）把多个命令编排为一个工作流。最简单的方案是 `Makefile`（不是只能给 C 用）：

```makefile
# Makefile
# .PHONY：声明 build/test/clean 是伪目标——即使目录里恰好有同名文件，make 也总是执行对应规则
...
## $\rm \S \, 21.2$ 依赖管理：项目的“购物清单”

### $\rm \S \, 21.2.1$ 直接依赖 vs 间接依赖
...
间接依赖的问题：`numpy` 使用的 `libopenblas` 版本有安全漏洞，但 `numpy` 还没有更新——你的项目间接受影响。`pip-audit` 和 `npm audit` 可以扫描已知漏洞，但你仍然需要等上游修复。

### $\rm \S \, 21.2.2$ 版本约束与冲突

在[软件、运行时、SDK 与包管理器](../02-终端与工具/09-软件运行时SDK与包管理器.md)中你见过 manifest 和 lockfile 的分工。版本约束写在你编辑的 manifest 中：
...
### $\rm \S \, 21.3.2$ C++ 测试框架速览

在[从竞赛 C++ 到工程 C++](../06-编程语言/15-C++从竞赛到工程.md)中你见过 Google Test 的基本语法。更多选择：

| 框架 | 特点 |
|---|---|
| **Google Test** | 工业标准，配套 GMock（mock 框架），文档丰富 |
| **Catch2**（github.com/catchorg/Catch2） | BDD 风格（`SCENARIO`），适合中小项目；v2 是单头文件，v3 改为库形式 |
| **doctest**（github.com/doctest/doctest） | 超轻量（编译比 Catch2 快很多），API 兼容 Catch2 |

### $\rm \S \, 21.3.3$ 测试替身：隔离被测对象
...
## $\rm \S \, 21.4$ CI：另一台干净机器上的自动检查

### $\rm \S \, 21.4.1$ 为什么需要 CI
...
### $\rm \S \, 21.4.2$ GitHub Actions 的最小 CI

在[GitHub：远程仓库与协作](20-GitHub使用与协作.md)中你见过 GitHub Actions 的基本概念。一个实际的三语言项目 CI：
...
§21.5 的工作流里“构建”只是一个框，这一节把它拆开。你在[第 1 章 计算机程序与开发全景](../01-基础观念/01-计算机程序与开发全景.md)见过完整链条：编译器把每个源文件编译成目标文件（`.o`），链接器再把目标文件和库连成可执行文件。目标文件是编译产物，可执行文件是最终产物，中间缺一种形态——**库**（library）：把编译好的目标文件打包，供多个程序复用。

问题出在复用上。论文管理器的 C++ 部分将来可能有三个程序：CLI、批量导入器、统计工具，都要用 `parse_paper()`。按第 15 章的做法，每个程序都要把 `paper.cpp` 一起编译链接，同一份代码被复制进三个可执行文件；改一行 `parse_paper()`，三个程序全部重新编译。库把“编译一次”和“到处使用”分开。按编译时复制还是运行时加载，库分为两类：静态库和动态库。

### $\rm \S \, 21.6.2$ 静态库：编译时复制
...
- `-c`：只编译、不链接——第 15 章“改一个 .cpp 只重编译它再链接”的前提就是它。
...
链接命令一样，产物却“欠债”——运行 `./paper-cli` 通常会报 `error while loading shared libraries: libpaper.so: cannot open shared object file`。这个报错你在[第 13 章 系统服务、日志与软件安装](../03-计算机系统/13-系统服务日志与软件安装.md)排过（`LD_LIBRARY_PATH` 就是那里的工具，见 §13.4.3），在这里它恰好是动态库代价的证据：**可执行文件不再自包含**，目标机器上必须有版本匹配的库，否则启动即失败。Windows 上这种版本冲突有个专门的名字：**DLL Hell**（DLL 地狱）——程序 A 更新了一个共享 dll，程序 B 突然无法启动。
...
### $\rm \S \, 21.6.4$ 你的程序已经在用动态库

用第 13 章的 `ldd` 看上面静态链接出的 `paper-cli` 依赖哪些库（Ubuntu 示例，具体输出随系统而异）：
...
列表里没有 `parse_paper()`——静态链接时它已经复制进二进制了。`libstdc++.so.6` 是 C++ 标准库实现，`libc.so.6` 是 C 运行库：你每次写 `#include <iostream>` 编译出的程序都在用它们，只是编译器默认帮你链好，你没看见。如果用 §21.6.3 的动态库链接，`ldd` 列表里会多出 `libpaper.so` 一行——这是“你的程序在用动态库”最直观的证据。Windows 没有 `ldd`，可以用 Process Explorer 或微软的 Dependencies 工具查看进程加载了哪些 dll。
...
### $\rm \S \, 21.7.1$ 版本号是兼容性承诺

§21.2.2 已经定义了语义化版本的三个数字，当时站在“选依赖”的角度读别人的版本号；这一节换到“发布自己的项目”的角度：你的库或程序每发一个新版本，都要回答所有使用者同一个问题——**升级会不会破坏我的代码？** 语义化版本用 `MAJOR.MINOR.PATCH`（如 `2.1.3`）把这个承诺编码进版本号：
...
注意“兼容”是维护者的承诺，不是自动成立的事实（§21.2.2 说过不要盲信版本号）。对使用者：MAJOR 要警惕，升级前先读 §21.8 的变更记录；对维护者：升 MAJOR 是强加给所有使用者的一次升级成本，动手前想清楚。

### $\rm \S \, 21.7.2$ 各生态的“允许范围”写法

版本号要能被工具使用，得翻译成“允许安装哪个范围”。npm 的 `package.json` 里（§21.2.1 的 express 就住在那里）：
...
`npm install` 会挑约束内当前最新的版本，并把实际选中的那一版写进 `package-lock.json`（[第 9 章](../02-终端与工具/09-软件运行时SDK与包管理器.md) §9.6 讲过 lockfile）——约束是意图，lockfile 是事实。Python 的 `requirements.txt` 没有 `^`，用显式区间表达同一意图：
...
## $\rm \S \, 21.8$ Changelog 与 Release Notes

### $\rm \S \, 21.8.1$ 用户升级前想知道什么
...
### $\rm \S \, 21.8.2$ Keep a Changelog 格式
...
### $\rm \S \, 21.8.3$ 与 Release Notes 的分工
...
### $\rm \S \, 21.9.1$ 同一份源码，两次构建

论文管理器 0.2.0 上线三个月后，用户报了一个只在旧版本出现的 bug。你 `git checkout` 当时的提交（第 19 章）重新构建、准备复现——但新构建出的程序和当时发布的不一致，bug 复现不出来。源码没变，变的是**构建环境**：当时的编译器、依赖、机器，都已经是三个月前的了。

于是有了一个标准：**可复现构建**（reproducible build）——同一份源码、同一台机器、不同时间构建，产物应该**逐字节相同**（byte-for-byte）。有了它，“这个二进制确实是这份源码编出来的”才可验证；旧版本出问题才能按原样重建。

### $\rm \S \, 21.9.2$ 什么在破坏可复现性
...
> **平台提示**：`sha256sum` 是 Linux/WSL 写法；Windows PowerShell 中没有这个命令，用 `Get-FileHash -Algorithm SHA256 app` 得到同样的 SHA-256 哈希，比较两次输出的 `Hash` 列即可。
...
### $\rm \S \, 21.9.3$ 两条捷径：lockfile 与 Docker

日常项目不需要手工解决全部因素，两条现成路径覆盖绝大多数情况。

**依赖锁定**。`package-lock.json`（npm）、`poetry.lock`（Python）——第 9 章和 §21.2 的 lockfile，作用就是让“装的依赖版本”可复现。依赖是产物差异的最大来源，锁住依赖就锁住大半。

**环境固定**。Docker（[第 30 章](../08-部署运维/30-Docker基础.md)）把整个构建环境——操作系统、编译器、依赖——封进镜像。只要 Dockerfile 不变，构建环境就不变，产物基本一致。这是通往可复现构建的捷径：你不必手工处理时间戳、路径这些细节，环境整体被固定了。
...
> 前置：项目已推送到 GitHub（第 20 章），且本地有可运行的 Python 测试（`pytest`）。成功标志：Actions 页面出现一次新运行，job 由运行中变为绿色通过；第 4 步的失败提交让运行变红、PR 旁出现红色叉号。清理：把故意改坏的测试改回来再提交一次；`.github/workflows/ci.yml` 是要长期保留的项目文件。

### $\rm \S \, 21.10.2$ 实践二：用 Makefile 编排多命令
...
> 前置：在 Bash/WSL 环境中运行——Windows 原生 PowerShell 没有 `make`，示例 `clean` 里的 `rm -rf` 也是 Bash 写法（可选替代：Task、Just）。成功标志：`make build`、`make test` 依次成功、第二次构建会跳过未变化的文件；第 3 步改坏源码后 `make build` 以非 0 退出并在报错处停下。清理：运行 `make clean` 删除构建产物，并把改坏的源文件改回。

### $\rm \S \, 21.10.3$ 实践三：扫描依赖漏洞
...
> 前置：先激活项目虚拟环境（第 9 章），避免把 pip-audit 装进系统 Python。成功标志：命令打印依赖表，并报告 `No known vulnerabilities found` 或列出有漏洞的包与漏洞 ID（发现漏洞时命令以非 0 退出，属正常行为）。清理：pip-audit 可留在虚拟环境中；不想保留时在同一个虚拟环境里 `pip uninstall pip-audit`。

### $\rm \S \, 21.10.4$ 实践四：把代码打包成静态库（需要 Linux 或 WSL）
...
> 前置：Linux 或 WSL（Windows 原生 MSVC 工具链的命令不同），当前目录是 `cpp-parser/`，且有第 15 章的 `main.cpp` 与 `paper.cpp`。成功标志：第 2 步 `./paper-cli` 的输出与直接编译所有源文件时一致；第 3 步 `ldd ./paper-cli` 的列表里出现 `libpaper.so`（直接运行可能报找不到库，这正是 §21.6.3 讲的代价）。清理：删除练习产生的 `*.o`、`libpaper.a`、`libpaper.so` 和 `paper-cli` 等构建产物，源码文件不要动。

### $\rm \S \, 21.10.5$ 实践五：写 CHANGELOG.md
...
> 前置：任意文本编辑器，在项目根目录操作。成功标志：文件中有一个 `[Unreleased]` 小节和一个带版本号与日期的已发布小节，三类条目各就各位，破坏性变更一行能看出“旧写法 → 新写法”。清理：`CHANGELOG.md` 是要提交进仓库的项目文件，无需清理——记得和代码一起提交。
...
### $\rm \S \, 21.14.1$ 自测答案 · 关键概念回顾
1. 直接依赖是项目直接使用的包，间接依赖是这些包再依赖的传递依赖；提交 lockfile 才能保证团队和 CI 安装的精确版本（含间接依赖）完全一致。
2. 2=MAJOR（不兼容的 API 修改）、1=MINOR（向后兼容的新功能）、3=PATCH（向后兼容的 bug 修复）；版本号是维护者的承诺，不应盲信。
3. 声明这些目标是伪目标（不是文件名）：即使目录里恰好有同名文件，make 也总是执行规则——build/test/clean 这类命令目标必须声明。
4. 静态库（`.a`/`.lib`）链接时把代码复制进可执行文件，自包含但每个程序一份拷贝、更新需重新链接；动态库（`.so`/`.dll`）运行时加载、多程序共享一份，但有版本兼容问题（DLL Hell）；内部代码优先静态，因为部署少一个“目标机器库版本”的变量。
5. `^1.2.3` 允许 `>=1.2.3, <2.0.0`（同 MAJOR 内最新）；`~1.2.3` 允许 `>=1.2.3, <1.3.0`（同 MINOR 内最新）；实际选中的版本由 lockfile 记录。

### $\rm \S \, 21.14.2$ 自测答案 · 应用与辨析
1. 构建系统按描述文件确定依赖关系、只重建变化的部分（CMake、Cargo）；任务运行器把多个命令编排成工作流（Makefile、Task、Just），可以依次调用多个构建系统。
2. 单元测试测单个函数/类（快、隔离、无外部依赖），集成测试测多个模块的协作；被测对象依赖数据库、网络等外部服务时用测试替身隔离，只验证被测逻辑。
3. 在干净虚拟机里从零安装依赖、构建、测试，排除本地缓存、未提交文件、全局工具等环境差异；本地通过只说明“在你的机器上通过”。

---

[Git：本地版本控制](19-Git与团队协作.md)处理了“代码版本”，[GitHub：远程仓库与协作](20-GitHub使用与协作.md)处理了“团队协作”，本章处理了“产物质量”。接下来，你需要把整个项目变成一个别人能通过浏览器访问的应用——这需要前端（HTML/CSS/JavaScript）和后端（Python API）协同工作。但在此之前，有一条分支路线：Node.js 和 npm 不仅是前端工具链的基石，也是理解现代 JavaScript 生态的钥匙。

> 你现在能：解释构建系统统一做什么，为项目接入依赖与锁文件，写最小单元测试，并理解 CI 在干净环境里验证提交的作用
