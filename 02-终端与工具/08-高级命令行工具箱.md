...
> **开始前自检**：本章假设你已经会：□ 会用第 7 章的工具（grep、curl、less、tail 等）；
> □ 理解重定向、管道与退出码；
> □ 在终端里连续运行过多条命令。
...
3. `+`、`?`、`()` 等元字符从字面字符变成语法；捕获组 `( )` 圈住的部分可以被 `grep -oE` 单独输出或由 sed 用 `\1` 引用。
4. 它是贪婪匹配，默认尽可能多地吃字符（`<.*>` 会从第一个 `<` 吃到最后一个 `>`）；在量词后加 `?` 变成 `.*?` 即非贪婪。注意 GNU grep 的 `-E` 不支持非贪婪，用 ripgrep 或 Python。
5. fd→find，bat→cat，fzf→交互式文件/历史查找（可增强 Ctrl+R），zoxide→cd（记住常用目录）。
