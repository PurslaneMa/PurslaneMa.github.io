...
C 是纯过程式——有 `struct` 但没有方法，数据和行为分离。C++ 把数据和行为捆绑成类——同一个 `paper.parse()` 方法的行为取决于 `paper` 的运行时类型（虚函数）。

---

## $\rm \S \, 14.3$ 面向对象：四个核心思想，不是"用 class 就叫 OOP"
...
```cpp
auto sum_positive = std::ranges::fold_left(
...
### $\rm \S \, 14.4.2$ 函数式的实际优势

**可测试性**：纯函数不需要 mock 数据库——相同输入永远相同输出。测试就是传入一个值、检查返回值。

**并发安全**：没有共享可变状态 → 没有数据竞争。在[操作系统、进程与线程](../03-计算机系统/09-操作系统进程与线程.md)中你花了很多时间理解 `std::mutex` 和死锁——函数式的不可变性从根源上消灭了这类问题。
...
## $\rm \S \, 14.5$ 声明式：描述"是什么"而不是"怎么做"

### $\rm \S \, 14.5.1$ SQL：最成功的声明式语言
...
你在[文本、编码、压缩包与常见数据格式](../02-终端与工具/05-文本编码压缩包与数据格式.md)中学过正则表达式也是声明式的：你描述模式，引擎决定匹配算法。

### $\rm \S \, 14.5.2$ 声明式在各领域的体现
...
在[日常系统管理：软件、用户、服务与日志](../03-计算机系统/11-系统服务日志与软件安装.md)中你写过的 systemd service 文件就是一种声明式配置——你声明"这个服务的可执行文件在哪、以谁的身份运行、崩溃后自动重启"，systemd 确保实际状态匹配声明。

---

## $\rm \S \, 14.6$ 类型系统：静态 vs 动态、强 vs 弱

### $\rm \S \, 14.6.1$ 两对容易混淆的维度
...
- Go 在编译型和脚本型之间取得了独特的平衡——编译产物是单文件二进制、启动极快、语法简单到半天能学会。

---

## $\rm \S \, 14.8$ 并发模型：不只多线程

在[操作系统、进程与线程](../03-计算机系统/09-操作系统进程与线程.md)中你接触了 `std::thread` 和互斥锁。不同语言对并发的支持各有侧重：

| 模型 | 代表 | 核心机制 |
|---|---|---|
| **OS 线程** | C++ `std::thread`、Java、Python `threading` | 操作系统调度，重量级 |
| **轻量线程/协程** | Go goroutine、Erlang process | 语言运行时调度，百万级并发 |
| **async/await** | Python asyncio、JavaScript、Rust tokio、C# | 单线程事件循环，I/O 密集场景 |
...
4. **语言本身的工程属性**：编译速度（Go 和 Rust 差 10 倍）、包管理成熟度（Rust Cargo 是标杆，C++ 仍在追赶）、学习曲线（Go 半周上手，Rust 三个月才能舒适使用 borrow checker）。

> 没有一个语言在所有维度上同时获胜。C++ 的控制力最强（你可以精确控制内存布局和分配策略），但需要你手动管理复杂性。Python 的开发速度最快，但运行时性能比 C++ 慢 10-100 倍。Rust 的内存安全最激进（编译器证明你的代码没有悬垂指针），但学习曲线最陡。Go 的并发模型最简洁，但在需要精细控制内存的场景不如 C++/Rust。
...
找一段你写的命令式循环代码（C++ 或 Python），用 `map`/`filter`/`reduce` 改写。感受两种表达方式的差异，但不做价值判断——哪种更清晰取决于具体逻辑。

### 实践二：体验类型系统差异

在 Python 中执行 `"hello" + 42`，观察 `TypeError`。在 JavaScript (Node.js) 中执行 `"hello" + 42`，观察结果。解释原因。然后分别在两种语言中执行 `typeof null`——你可能会对结果感到意外。

### 实践三：安装一门陌生语言，只写 Hello World

从上面任何一门你没用过的语言中选择一个（建议 Go 或 Rust），安装运行时，阅读官方 "Getting Started"，运行 Hello World。目的不是学语言，而是体验不同语言的工具链——Go 的一个 `go build` 产出单文件二进制的体验，和 C++ 的 CMake + 编译器 + 链接器链条形成对比。
...
> 命令式告诉计算机"怎么做"——一步步修改状态的语句序列（C++ 的 for 循环累加）；声明式描述"我要什么"（SQL 的 SELECT、正则表达式），怎么做由引擎决定。

2. 面向对象的四个核心概念是什么？"组合优于继承"在什么场景下成立？
> 封装、继承、多态、抽象；当你不确定两个类之间真的有 is-a 关系时用组合更安全——纯粹为了复用代码而继承（如 Stack 继承 Vector）会导致接口臃肿。

3. 函数式编程的两个核心原则是什么？纯函数为什么容易测试？
> 不可变数据 + 纯函数（相同输入永远相同输出、无副作用）；纯函数测试只需传值、检查返回值，不需要 mock 数据库或网络。

4. C++ 是静态还是动态类型？强还是弱？Python 呢？JavaScript 呢？
> C++ 静态强类型；Python 动态强类型（"hello" + 42 报 TypeError）；JavaScript 动态弱类型（"hello" + 42 得到 "hello42"）。

5. 编译型和解释型语言在启动速度和峰值性能上有什么区别？JIT 处于哪个位置？
> 编译型启动快、峰值性能高；解释型启动快但长期运行慢；JIT 先解释执行（启动快），再对热点代码编译（长期运行接近编译型）——Java、V8 属于这一类。

## $\rm \S \, 14.13$ 应用与辨析

6. Go 的 goroutine 和 C++ 的 `std::thread` 在模型上有什么区别？
> std::thread 是操作系统线程，由内核调度、创建成本高，数量到百万级不可行；goroutine 是语言运行时调度的轻量协程，内存开销小，可以支撑百万级并发。

7. 什么时候该用函数式风格而不是命令式风格实现同一个功能？
> 当逻辑是"把集合变换成另一个集合"（筛选、映射、聚合）且没有副作用时，函数式表达更短、更不容易出错（如用列表推导替代 for 循环累加）；当任务本质是按顺序修改状态（打印日志、写文件、状态机）时，命令式更直观。

---

接下来，为了真正让论文资料管理器跑起来，你需要一门能快速编写 API、处理数据和管理依赖的语言——Python。下一章（第 15 章）假设你会 C++，直接带你进入 Python 的思考方式，而不是重复"变量是什么""循环怎么写"。
