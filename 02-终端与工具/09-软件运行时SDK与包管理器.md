...
> **开始前自检**：本章假设你已经会：□ 会用终端运行命令与 Python 脚本；
> □ 理解 PATH：输入程序名时 Shell 去哪里找它（第 2 章）；
> □ 遇到过"明明装了却找不到/版本不对"的困惑。
...
2. 系统包管理器管理操作系统层面的软件（git、g++、curl），需要管理员权限；语言包管理器管理某语言的第三方库，装到项目或用户目录；工具用系统的装，项目依赖用语言的装。
...
5. 预编译包（wheel）已针对当前平台编译好，安装快、不需要本地编译器；从源码构建需要本地编译器（如 VS Build Tools）和相关库，慢且更容易失败。
6. 安装过程可能执行包作者编写的脚本（setup.py、postinstall），这些脚本可以做任何事；包管理器只做基础扫描，所以只装来源明确的包、警惕拼写仿冒（typosquatting）。

### 自测答案 · 应用与辨析
7. 用 `pip` 装的包装进了一个 Python 的环境，而 `python` 命令实际是另一个 Python，`import` 自然找不到；用 `python -m pip install` 保证 pip 属于当前 `python`。
8. 先装运行时（系统包管理器）→ 创建虚拟环境 → 按 manifest/lockfile 安装依赖（`pip install -r` / `npm ci`）→ 验证关键包能导入、服务能启动。
